...
SELECT
    ...
    ri.created_at,
    ri.updated_at,
    COALESCE(r.rid, NULLIF(ri.room_rid, ''), ''),
    COALESCE(r.name, NULLIF(ri.room_name, ''), 'Deleted room'),
    COALESCE(r.avatar_url, ri.room_avatar_url),
    COALESCE(r.default_avatar_key, NULLIF(ri.room_default_avatar_key, ''), 'room-1'),
    COALESCE(r.visibility, NULLIF(ri.room_visibility, ''), 'private'),
    COALESCE(r.join_policy, NULLIF(ri.room_join_policy, ''), 'closed')
